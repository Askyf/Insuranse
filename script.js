...
function initProducts() {
    ...
    // Try to load products from external file first, then fallback to inline
    loadProducts()
        .then(products => {
...
async function loadProducts() {
    try {
        // First try to load from external products.js file
        ...
        // Assuming products.js has: const products = [...];
        const match = scriptContent.match(/const products = (\[.*?\]);/s);
        if (match && match[1]) {
            const products = eval(match[1]);
            return products;
        } else {
            throw new Error('Could not parse products from products.js');
        }
    } catch (error) {
        console.log('Falling back to inline products data');
        
        // Fallback to inline products data
